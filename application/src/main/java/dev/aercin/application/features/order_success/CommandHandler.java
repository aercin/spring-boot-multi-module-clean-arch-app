...
@Component("orderSuccessCommandHandler")
public class CommandHandler implements RequestHandler<Command, Result> {
    ...
    public Result handle(Command request) {
        ...
        if(uow.getInboxRepo().findByIdAndConsumerType(request.getOrderId(),"orderSuccessCommandHandler").isPresent())
        {
        ...
        if (orderOpt.isPresent()) {
            try {
                ...
                inboxMsg.setConsumerType("orderSuccessCommandHandler");
